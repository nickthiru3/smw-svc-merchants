...
The microservice uses a **modular, layered configuration approach**:

1. **Domain-Specific Configs** - Separate files for each concern:
...
2. **Type Definitions** (`config/types.ts`) - TypeScript interfaces
3. **Validation Schema** (`config/schema.ts`) - Zod runtime validation
4. **Base Configuration** (`config/default.ts`) - Aggregates domain configs
5. **Environment Overrides** (`config/{env}.ts`) - Environment-specific overrides
6. **Runtime Variables** (Environment variables) - Dynamic values at deployment time
...
**Purpose**: Aggregates domain-specific configs and loads environment-specific overrides.

**⚠️ IMPORTANT: DO NOT edit this file directly for configuration changes!**
...
- Imports and aggregates domain-specific configurations
...
function loadConfig(): IConfig {
  ...
  // Create default config from domain-specific configs
...
function createDefaultConfig(envName: string): IConfig {
  return {
    ...
    // Domain-specific configurations (imported)
    ...
    github: createGitHubConfig(envName),
    aws: createAwsConfig(envName),
...
### 3.6. Domain-Specific Configuration Files

#### `config/types.ts` - Type Definitions

**Purpose**: Central location for all configuration interfaces.
...
- `ConfigSchema` - Main Zod validation schema
- Validates all configuration fields
- Enforces environment-specific requirements

#### `config/service.ts` - Service Configuration

**Purpose**: Service metadata (name, display name).

**Environment Variables**:

- `SERVICE_NAME` - Technical service name
- `SERVICE_DISPLAY_NAME` - Human-readable name
...
**See**: [Adding Endpoints - Part 2](./adding-endpoints-part-2-api-gateway.md) for usage.

#### `config/resources.ts` - Resource Naming

**Purpose**: Prefixes for AWS resource names.
...
- `PERMISSIONS_ENABLED` - Enable OAuth permission scopes

#### `config/github.ts` - GitHub Configuration
...
- `CODESTAR_CONNECTION_ID` - AWS CodeStar connection
...
### 8.1. Determine Configuration Scope

**Question**: Is this configuration domain-specific or environment-specific?

- **Domain-specific** (applies to all environments): Create a new config file
- **Environment-specific** (varies by environment): Add to environment override files

### 8.2. For Domain-Specific Configuration

#### Step 1: Create new config file

**File**: `config/my-feature.ts`
...
#### Step 3: Add to Zod schema in `config/schema.ts`
...
#### Step 4: Import in `config/default.ts`
...
### 8.3. For Environment-Specific Configuration
...
const defaultConfig: IConfig = {
  service: {
    name: "svc-merchants", // Don't edit here!
  },
};
```

### 9.2. Environment Variables Over Hardcoded Values
...
### 9.4. Fail Fast for Required Values
...
### 9.5. Use Partial<IConfig> for Overrides
...
### 9.7. Don't Import Config in Handlers
...
### 9.8. Use SSM for Service Discovery
