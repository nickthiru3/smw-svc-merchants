...
3. [Development Workflow](#3-development-workflow)
   ...
   - 3.5. [Step 4: Implement Data Access Layer](#35-step-4-implement-data-access-layer)
   - 3.6. [Step 5: Implement Lambda Handlers](#36-step-5-implement-lambda-handlers)
   - 3.7. [Step 6: Wire to API Gateway](#37-step-6-wire-to-api-gateway)
   - 3.8. [Step 7: Run All Tests Locally](#38-step-7-run-all-tests-locally)
   - 3.9. [Step 8: Deploy to Dev Environment](#39-step-8-deploy-to-dev-environment)
   - 3.10. [Step 9: E2E Testing (Optional)](#310-step-9-e2e-testing-optional)
   - 3.11. [Step 10: Add Monitoring](#311-step-10-add-monitoring)
   - 3.12. [Step 11: Document & Track](#312-step-11-document--track)
...
### 2.1. Treat All Guides as Placeholders
...
### 2.2. Generic vs App-Specific Documentation
...
### 2.3. Bottom-Up Implementation Approach
...
- **Entity file**: `docs/project/specs/entities/[entity].md`
  - Main Table Structure section
...
### 3.5. Step 4: Implement Data Access Layer

**Purpose**: Create the data access layer for DynamoDB operations

**Guide**: [Data Access Layer](./data-access.md)

**Story Artifacts**:

- **Entity file**: `docs/project/specs/entities/[entity].md`
  - Main Table Structure section (for primary key design)
  - Global Secondary Indexes section (for GSI queries)
  - Access Patterns table (for query operations)
  - Attribute Definitions section (for TypeScript interfaces)
- **Actions & Queries**: `docs/project/specs/stories/[actor]/[story]/actions-queries.md`
  - Queries section (for read operations)
  - Expected Output examples (for response structure)

**Implementation**:

1. **Review data model** from `docs/project/specs/entities/[entity].md`
2. **Create TypeScript interfaces** in `src/types/[entity].ts`
   - Domain entity (application-level)
   - DynamoDB item (with PK/SK/GSI attributes)
3. **Implement data access functions** in `src/lib/data-access/[entity].ts`
   - Create, read, update, delete operations
   - Query functions for access patterns
   - Mapper functions (domain ↔ DynamoDB)

**Testing**: Write unit tests for data access functions
...
### 3.6. Step 5: Implement Lambda Handlers
...
- **Actions & Queries**: `docs/project/specs/stories/[actor]/[story]/actions-queries.md`
  - Queries section (for query handlers)
  - Actions section (for command handlers)
  - Inputs (for request validation)
  - Expected Output (for response formatting)
  - Error Cases (for error handling)
- **Story Card**: `docs/project/specs/stories/[actor]/[story]/story-card-[number].md`
  - Business Rules section (for validation logic)
  - Implementation Notes & Assumptions

**Implementation**:

1. **Create handler directory** in `src/handlers/[endpoint]/`
2. **Implement handler logic** using data access layer
3. **Add request/response validation** with Zod schemas
4. **Handle errors** and return proper HTTP responses

**Testing**: Write handler tests with mocked data access layer
...
### 3.7. Step 6: Wire to API Gateway
...
- **OpenAPI Spec**: `docs/project/specs/api/resources/[resource]/[operation].yaml`
  - Path and method definition
  - Request parameters
  - Request/response schemas
  - Status codes and error responses
- **Actions & Queries**: `docs/project/specs/stories/[actor]/[story]/actions-queries.md`
  - API Endpoint section (for endpoint path and method)
...
### 3.8. Step 7: Run All Tests Locally
...
### 3.9. Step 8: Deploy to Dev Environment
...
### 3.10. Step 9: E2E Testing (Optional)
